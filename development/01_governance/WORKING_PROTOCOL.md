...
metadata. Neither control suppresses direct current-valid-result routing. That
precedence applies over directly proven invalid or validly contained non-valid
residue; it does not turn an uncontained uninspectable candidate into invalid
residue. Any such epistemically unknown candidate remains fail-closed even when
one current-valid result is visible.
