...
**Date:** 2026-08-26; revised 2026-08-27 under WP-014
**Supersedes:** none
**Superseded by:** none
...
9. **Validity override and contained states.** Containment never converts a head
   ...
   not an absence claim; later inspectability and validity route normally.
...
fixed defect. Containment adds state only after repeated invalid movement is
observed and binds that state to the candidate stream plus complete canonical
key. The validity override is the critical non-suppression boundary: containment
removes repeated invalid movement's blocking authority, not a later valid
result's routing eligibility. This is smaller than a lock/lease or trust system
and directly addresses the demonstrated denial class. The final re-check and
WP-local bridge remain unchanged for publication timing and provisional rollout.
...
reliably; containment hides or delays a later valid result; containment records
cause false exclusions or operational burden; distinct PR creation reproduces a
broader unbounded denial class; the WP-local bridge cannot protect generic
cold-start; the residual final-check edge reproduces material duplicate work; or
a platform-native atomic handoff becomes available with less state and equal
...
**PASS** and WP-013 **Requires repair** permanently bound only to it. WP-014
publishes this revised proposal from current canonical development head in a new
superseding draft PR; PR #16 is not amended or merged. The new target requires
fresh verification and fresh adversarial re-review and does not accept this ADR.
