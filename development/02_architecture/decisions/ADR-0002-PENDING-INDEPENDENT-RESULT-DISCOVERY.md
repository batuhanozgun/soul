...
**Date:** 2026-08-26; revised 2026-08-27 under WP-014/WP-017 and 2026-08-28 under WP-020
**Supersedes:** none
**Superseded by:** none
...
PR #22 exact target `5bd0db27fc3df368c9e112f01b7eed49a64402ab`
implemented candidate-set containment and restored precedence over inspectable
invalid residue. WP-018 issued exact-target **PASS**, but WP-019 later issued
**Requires repair** and preserved F-AR-008: the target also put an uncontained
uninspectable candidate into its generic invalid class, then routed one visible
current-valid result first. Because metadata is only a locator, that candidate
was epistemically unknown and could conceal a second current result.

The immediate cause is result routing before inspection-blocker routing in the
mixed visible/unknown state. The system cause is a collapsed non-current class
that treated directly proven invalid, validly contained non-valid and
uncontained uninspectable candidates as equivalent residue.
...
- Exactly one current-valid result must route before invalid residue across the
  ...
  conflict.
- An uncontained candidate whose immutable head or required records cannot be
  inspected remains epistemically unknown and blocks in every mixed state.
...
### Keep validity-first ordering and special-case one inaccessible test

Rejected under WP-020. A route-specific exception would leave the underlying
state collapse in normative text, templates or later consumers and would not
establish why the candidate is invalid.

### Three-way epistemic classification with unknown-before-result routing

Chosen for WP-020. Preserve the existing resolution and containment controls,
but classify the full discovered set as directly validated current/invalid,
exactly resolved or contained non-valid, or uncontained uninspectable. A sole
current-valid result precedes the first two non-valid residue classes. The last
class blocks before result routing because it may conceal another current-valid
result. No new authority or control record is introduced.
...
- WP-017 producer regression evidence and corrected executable decision-table
  model, used only as producer evidence.
- immutable WP-019 adversarial re-review, SESSION-0034 and F-AR-008's
  visible-valid plus uncontained-uninspectable counterexample;
- WP-020 producer regression and deliberate unknown-after-result mutation,
  used only as producer evidence.
...
4. **Epistemic classification before result routing.** Directly validate every
   inspectable same-WP head, then distinguish directly proven invalid, exactly
   resolved or contained non-valid, and uncontained uninspectable candidates.
   An uncontained uninspectable candidate is unknown and blocks in every mixed
   state. If no unknown remains, exactly one current-valid result routes before
   directly proven invalid or contained non-valid residue. Multiple valid
   current results remain a conflict; the Integrator preserves them and
   canonically activates a fresh attempt/key.
5. **Bounded recovery.** Invalid/stale/malformed candidates recover after an exact-head resolution is integrated canonically. Uncontained uninspectable cases recover only when inspection becomes possible or an exactly applicable canonical control already proves/contains that immutable candidate state. Repository-wide discovery unavailability remains blocked. Closing a PR alone has no resolution effect.
...
WP-020 narrows that boundary to evidence actually known. Direct inspection or
an exactly applicable canonical control can establish non-valid residue;
inability to inspect cannot. Routing unknown before a visible result therefore
preserves the same valid-result precedence without allowing it to suppress a
possible second result/conflict. This is a decision-table/type correction inside
the existing control architecture, not a new control plane.
...
- one current-valid result routes before directly proven invalid or contained
  non-valid residue, while visible-valid plus uncontained unknown fails closed;
...
- uncontained candidate-specific inaccessibility remains an explicit unknown
  blocker even when another result is visible;
...
17. prove visible-valid plus uncontained-uninspectable and directly proven
    invalid plus uncontained-uninspectable both block; prove the same candidate
    later inspectable-valid exposes conflict and later inspectable-invalid
    preserves sole-valid routing;
18. prove exact-head resolution, stream containment and candidate-set
    containment each permit inaccessible non-valid residue without turning it
    into validity or suppressing later direct validation;
19. deliberately mutate the executable model to route a visible result before
    an uncontained unknown and observe the mixed-state suite fail red.
...
available with less state and equal authority separation; or any consumer again
conflates uncontained inspection failure with proven non-valid residue.
...
PR #22 remains the immutable rejected WP-017 candidate at `5bd0db27...`, with
WP-018 **PASS** and WP-019 **Requires repair**/F-AR-008 permanently bound only
to it. WP-020 publishes a new superseding draft material PR from the later
canonical development head; PR #22 is not amended or merged. The changed target
requires fresh verification and fresh adversarial re-review and does not accept
this ADR.
