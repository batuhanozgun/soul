...
## Resolution effect and boundary

This record excludes only the exact tuple `(repository, PR number, candidate head SHA)` from repeated pending-result blocking after this record is present on the canonical development branch. It does not apply after candidate head movement. It does not reinterpret or accept any result, change the active key by itself, or authorise verifier/reviewer self-transition.

## Recovery / reopen condition

State how work resumes and what reopens inspection, including candidate head movement, new evidence, a changed canonical key, or proof that this classification was wrong.
