...
current-result validation was evaluated before routing. Keep these classes
explicit and separate for every same-scope candidate:

- directly inspected current-valid or invalid;
- exactly resolved or validly contained non-valid;
- uncontained uninspectable, which is epistemically unknown rather than invalid.

Repository-wide discovery failure or any candidate in the third class blocks
before result routing, including when one or more current-valid results are
visible. If no unknown remains, multiple current-valid results remain conflict
and exactly one current-valid result routes to Integrator before directly proven
invalid or resolved/contained non-valid residue.

An exact-head resolution may cover a candidate that becomes inaccessible only
when the resolution was canonically integrated from an earlier direct
inspection of that same immutable head. Inaccessibility itself cannot create or
justify the resolution. The same proof boundary applies to candidate-specific
inaccessibility under either containment mode.
...
An inaccessible head outside this exact stream identity remains uncontained
unknown and blocks even if another current-valid result is visible.
...
An inaccessible candidate outside this exact repository + complete-key scope
remains uncontained unknown and blocks when it is otherwise in scope, even if
another current-valid result is visible.
