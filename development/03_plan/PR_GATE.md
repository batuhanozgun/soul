...
`COLD_START.md` checks candidates after canonical state discovery and repeats the live check immediately before independent-role commitment. Repository-wide discovery failure and any uncontained candidate-specific inspection failure fail closed even when a current-valid result is already visible.
...
- every inspectable later head is still directly validated; a current valid head
  bypasses directly proven invalid or contained non-valid residue and routes
  normally, and multiple current valid results remain a conflict;
...
- candidate-set containment survives fresh PR identity creation and is the
  ...
  valid result routes before directly proven invalid or contained non-valid
  residue and multiple current valid results remain a conflict;
...
- an uncontained uninspectable candidate is epistemically unknown, not invalid
  residue; locator metadata, a prior invalid head or one visible result cannot
  suppress it, so it blocks until direct inspection or an exactly applicable
  canonical control establishes the bounded non-valid state;
...
- unavailable repository-wide discovery remains blocked until available,
  regardless of any already visible result.
